...
export function DailyGoals({ goals, onGoalsUpdate }: DailyGoalsProps) {
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    if (passed) {
      ...
      setVerificationSuccess({ show: true, success: true, title: message, message: subMessage, goal: verifyingGoal });
    } else {
      setVerifyingGoal(null);
      setVerificationSuccess({ show: true, success: false, title: message, message: subMessage, goal: verifyingGoal });
    }
  };
  ...
  const handleStartGoal = async (goal: DailyGoal) => {
    try {
      await api.put(`/api/goals/${goal.id}`, { started_at: new Date().toISOString() });
      onGoalsUpdate();
    } catch (error) {
                ...
                >
                  <div className="flex-shrink-0">
                    {goal.is_completed ? (
                      ...
                      <button
                        ...
                        disabled={goal.duration_minutes > 0 && !goal.started_at}
                      >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {goal.is_recurring && <Repeat className="w-3 h-3 text-cyan-500/70" />}
                    </div>
            ...
            >
              <div className="text-center mb-6">
                <div className="w-12 h-12 bg-cyan-500/20 rounded-full flex items-center justify-center mx-auto mb-3">
                  {verifyingGoal.category === 'Coding' ? <Box className="w-6 h-6 text-cyan-400" /> : <Video className="w-6 h-6 text-cyan-400" />}
                </div>
                <h3 className="text-xl font-bold text-white mb-1">Verify Completion</h3>
                <p className="text-gray-400 text-sm">
                  {verifyingGoal.category === 'Coding' ? "How many coding questions did you solve?" : "How many lectures did you attend?"}
                </p>
              </div>

              <form onSubmit={handleVerifySubmit}>
                <input
                  type="number" min="0" value={verificationInput}
                  onChange={(e) => setVerificationInput(e.target.value)}
                  className="w-full px-4 py-3 bg-black/50 border border-gray-700 rounded-xl text-white text-center text-2xl font-bold mb-6 focus:ring-2 focus:ring-cyan-500 outline-none"
                  autoFocus
                />
                <div className="flex gap-3">
                  <button type="button" onClick={() => setVerifyingGoal(null)}
                    className="flex-1 py-3 bg-gray-800 text-white rounded-xl font-medium hover:bg-gray-700 transition-colors">
                    Cancel
                  </button>
                  <button type="submit"
                    className="flex-1 py-3 bg-gradient-to-r from-cyan-600 to-blue-600 text-white rounded-xl font-medium hover:from-cyan-500 hover:to-blue-500 transition-colors">
                    Verify
                  </button>
                </div>
              </form>
        ...
        {verificationSuccess.show && (
          ...
          >
            ...
            >
              ...
              >
                {verificationSuccess.success ? <CheckCircle2 className="w-10 h-10 text-white" /> : <Target className="w-10 h-10 text-white" />}
              </motion.div>
              ...
              <p className="text-gray-300 text-lg mb-8 relative z-10 leading-relaxed">{verificationSuccess.message}</p>

              <button
                onClick={handleCloseSuccess}
                className={`w-full py-4 rounded-xl font-bold text-lg shadow-xl relative z-10 transition-transform active:scale-95 ${verificationSuccess.success ? 'bg-gradient-to-r from-green-500 to-cyan-600 text-white' : 'bg-gray-800 text-white hover:bg-gray-700'}`}
              >
