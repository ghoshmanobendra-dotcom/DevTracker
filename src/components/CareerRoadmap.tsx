...
export function CareerRoadmap({ careerPathId, userId }: CareerRoadmapProps) {
    ...
    const loadProgress = async () => {
        try {
            const { data } = await api.get('/api/career-progress');
            setCompletedTopics(new Set(data.map((d: any) => d.topic_id)));
        } catch (e) {
            console.error('Error loading career progress:', e);
        }
        setLoading(false);
    };
    ...
    const toggleTopic = async (topicId: string) => {
        ...
        try {
            if (isCompleted) {
                await api.delete(`/api/career-progress/${topicId}`);
            } else {
                await api.post('/api/career-progress', { topic_id: topicId });
            }
        } catch (e) {
            // Rollback on error
    ...
    const updateCareerPath = async (id: string) => {
        try {
            await api.put('/api/profile', { career_path: id });
            window.location.reload();
        } catch (e) {
            console.error('Error updating career path:', e);
        }
    };
            ...
            <div className="bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-2xl p-6 text-center">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {CAREER_PATHS.map(path => (
                        <button key={path.id} onClick={() => updateCareerPath(path.id)}
