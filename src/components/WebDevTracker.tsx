...
export function WebDevTracker({ projects, onProjectsUpdate, sectionTitle = "Web Projects" }: WebDevTrackerProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setFormData({ projectName: '', description: '', techStack: '', repoUrl: '', demoUrl: '', status: 'Planned' });
            setShowForm(false);
            onProjectsUpdate();
        } catch (error) {
        ...
        <div className="bg-gray-900/40 backdrop-blur-xl border border-white/5 rounded-3xl p-6 shadow-xl relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-8 relative z-10">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Build and track your portfolio</p>
                </div>
                <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
                    ...
                    {showForm ? 'Close' : (<><Plus className="w-4 h-4" />Add Project</>)}
                </motion.button>
            </div>
                    ...
                    <motion.form initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                        ...
                        <div className="p-4 bg-gray-800/30 rounded-2xl border border-white/5 space-y-3">
                            <input type="text" value={formData.projectName} onChange={(e) => setFormData({ ...formData, projectName: e.target.value })}
                                ...
                                className="w-full px-4 py-3 bg-gray-900/50 border border-gray-700/50 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-blue-500/50 focus:ring-1 focus:ring-blue-500/50 transition-all" required />
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <input type="url" value={formData.repoUrl} onChange={(e) => setFormData({ ...formData, repoUrl: e.target.value })}
            ...
            <div className="space-y-4 relative z-10">
                {projects.length === 0 ? (
                    ...
                    projects.map(project => (
                        <motion.div layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} key={project.id}
                            ...
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex items-center gap-2">
                                    <select value={project.status} onChange={(e) => handleStatusChange(project, e.target.value)}
                                        ...
                                        {statuses.map(status => <option key={status} value={status} className="bg-gray-900 text-white">{status}</option>)}
                                    </select>
