...
export function CodingTracker({ problems, onProblemsUpdate }: CodingTrackerProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ sectionName: 'DSA Practice', problemName: '', problemLink: '', difficulty: 'Medium', status: 'Unsolved', youtubeSolution: '', resourceUrl: '', notes: '' });
      setShowForm(false);
      onProblemsUpdate();
    } catch (error) {
  ...
  const handleStatusChange = async (problem: CodingProblem, newStatus: string) => {
    try {
      await api.put(`/api/problems/${problem.id}`, {
        status: newStatus,
    ...
    <div className="bg-gray-900/50 backdrop-blur-xl border border-gray-800 rounded-2xl p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
        ...
        <div className="flex gap-2">
          {['All', 'Easy', 'Medium', 'Hard'].map((diff) => (
            <button key={diff} onClick={() => setFilterDifficulty(diff as any)}
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 bg-gray-800/50 rounded-xl border border-gray-700 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <input type="text" value={formData.sectionName} onChange={(e) => setFormData({ ...formData, sectionName: e.target.value })}
              placeholder="Section (e.g., DP Practice)"
              className="px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-purple-500" required />
            <input type="text" value={formData.problemName} onChange={(e) => setFormData({ ...formData, problemName: e.target.value })}
              placeholder="Problem name"
              className="px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-purple-500" required />
          </div>
          <input type="url" value={formData.problemLink} onChange={(e) => setFormData({ ...formData, problemLink: e.target.value })}
          ...
          <input type="url" value={formData.youtubeSolution} onChange={(e) => setFormData({ ...formData, youtubeSolution: e.target.value })}
            placeholder="YouTube solution link (optional)"
            className="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-purple-500" />
          <input type="url" value={formData.resourceUrl} onChange={(e) => setFormData({ ...formData, resourceUrl: e.target.value })}
      ...
      {filteredProblems.length === 0 ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">{problems.length === 0 ? "No problems tracked yet. Add one to start!" : "No matches found."}</p>
        </div>
      ) : (
        <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
          {filteredProblems.map((problem, index) => (
            <div key={problem.id} className="group p-4 bg-gray-800/50 border border-gray-700 hover:border-gray-600 rounded-xl transition-all">
              ...
              {(problem.problem_link || problem.youtube_solution || problem.resource_url) && (
                <div className="flex items-center gap-2 mb-2">
                  ...
                  {problem.youtube_solution && (
                    <a href={problem.youtube_solution} target="_blank" rel="noopener noreferrer"
                      ...
                      <Youtube className="w-3 h-3" />Solution
                    </a>
                  )}
              ...
              {problem.notes && <p className="text-sm text-gray-400 mt-2">{problem.notes}</p>}
            </div>
          ))}
        </div>
      )}
