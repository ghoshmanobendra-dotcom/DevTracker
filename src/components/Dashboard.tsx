...
export function Dashboard() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // Cleanup old goals
      await api.delete(`/api/goals/cleanup?olderThan=${sevenDaysAgoStr}`).catch(() => {});

      const [profileRes, goalsRes, problemsRes, projectsRes, notesRes, shortcutsRes, scoresRes] = await Promise.all([
        api.get('/api/profile'),
        api.get(`/api/goals?date=${today}`),
        api.get('/api/problems'),
        api.get('/api/projects'),
        api.get('/api/notes'),
        api.get('/api/shortcuts'),
        api.get('/api/scores'),
      ]);
  ...
  const handleGoalsUpdate = useCallback(async () => {
    ...
    const { data: updatedGoals } = await api.get(`/api/goals?date=${today}`);
